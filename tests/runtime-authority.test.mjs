...
test("runtime policy is a routing card and delegates detail to canonical owners", async () => {
  ...
  assert.match(text, /Agent Executor[\s\S]*exact Task Envelope[\s\S]*integration owner/i);
  assert.match(text, /integration owner[\s\S]*同一时间只能有一个[\s\S]*accountable writer/i);
  assert.match(text, /current-tool Builder[\s\S]*bounded local commit series/i);
  assert.match(text, /exclusive isolated worktree/i);
  assert.match(text, /Builder 不因 commit grant 自动成为 integration owner[\s\S]*Executor[\s\S]*(?:adopt|采用)/i);
  assert.match(text, /integration ownership[\s\S]*本身不是 Git grant/i);
  assert.match(text, /integration adoption: allowed[\s\S]*exact inbound commit list/i);
  assert.match(text, /reconciliation edit\/commit[\s\S]*Builder Role[\s\S]*Builder Git Envelope/i);
  assert.match(text, /Scout[\s\S]*Architect[\s\S]*Reviewer[\s\S]*Verifier[\s\S]*cc\/cx external profiles[\s\S]*must not[\s\S]*(?:stage|commit)/i);
...
test("adapter snippets activate the compact policy without copying runtime files", async () => {
  ...
  assert.match(combined, /Agent Executor[\s\S]*integration owner[\s\S]*non-protected[\s\S]*task branch/i);
  assert.match(combined, /current-tool Builder[\s\S]*bounded local commit series[\s\S]*Task Envelope[\s\S]*exclusive isolated worktree/i);
  assert.match(combined, /Scout[\s\S]*Architect[\s\S]*Reviewer[\s\S]*Verifier[\s\S]*cc[\s\S]*cx[\s\S]*must not stage or commit/i);
